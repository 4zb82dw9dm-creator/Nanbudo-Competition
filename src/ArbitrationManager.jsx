...
function ArbitrationManager({ competition, onUpdateCompetition }) {
  ...
  function saveMatch(result) {
    const winnerId = selectedMatch.discipline === "kata" ? selectedMatch.akaId : result.vainqueur === "aka" ? selectedMatch.akaId : result.vainqueur === "shiro" ? selectedMatch.shiroId : null;
  ...
  if (selectedPool && selectedMatch) {
    ...
    return <div className="arbitration-manager"><button className="back-button" onClick={() => setSelected(null)}>← Retour aux matchs</button>{selectedMatch.discipline === "kata" ? <KataSheet key={selectedMatch.id} match={matchProps} availableKatas={competition.katas || competition.availableKatas || []} onSave={saveMatch} /> : <MatchManager key={selectedMatch.id} match={matchProps} onSave={saveMatch} />}</div>;
  }
  return <div className="arbitration-manager"><div className="manager-header"><div><p className="surtitle">ARBITRAGE DIRECT</p><h2>Arbitrage</h2><p>Le clic sur un match ouvre immédiatement la feuille officielle Kata ou Combat, sans page intermédiaire.</p></div></div>{pools.length === 0 ? <div className="empty-state"><h3>Aucun match disponible</h3><p>Générez et validez les poules avant l'arbitrage.</p></div> : <div className="competition-list">{pools.flatMap((pool) => (pool.matches || []).map((match) => { const category = getCategory(pool.categoryId); const isKata = match.discipline === "kata"; const competitor = getCompetitor(match.competitorId || match.akaId); const winner = match.winnerId ? getCompetitor(match.winnerId) : null; return <article className={`competition ${match.statut === "Terminé" ? "competition-terminee" : ""}`} key={match.id}><div><p className="surtitle">{disciplineLabel(match.discipline)} · {category?.nom}</p><h3>#{match.ordre} Tatami {match.tatami} {match.horaire && `· ${match.horaire}`}</h3>{isKata ? <p>Passage : {competitor?.nom} {competitor?.prenom} · {competitor?.club || "Club non renseigné"}{match.finalScore ? ` · Note ${Number(match.finalScore).toFixed(2)}` : ""}</p> : <p>AKA {getCompetitor(match.akaId)?.nom} {getCompetitor(match.akaId)?.prenom} vs SHIRO {getCompetitor(match.shiroId)?.nom} {getCompetitor(match.shiroId)?.prenom}</p>}{winner && !isKata && <p>Vainqueur : {winner.nom} {winner.prenom}</p>}</div><button className="manage-button" onClick={() => setSelected({ poolId: pool.id, matchId: match.id })}>{match.statut === "Terminé" ? "Modifier" : "Arbitrer"}</button></article>; }))}</div>}</div>;
}
