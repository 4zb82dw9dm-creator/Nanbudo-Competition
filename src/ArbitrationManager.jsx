...
import { effectiveRefereeAssignments, refereeSlotsForDiscipline, replaceCompetitionReferee, replaceMatchReferee, TABLE_REFEREE_SLOTS } from "./refereeTeamRules";
...
function ArbitrationManager({ competition, onUpdateCompetition }) {
  ...
  function updateMatchReferee(match, slot, refereeId) {
    const baseTeam = competition.refereeAssignments?.[match.tatami] || {};
    let refereeOverrides = { ...(match.refereeOverrides || {}) };
    if (refereeId === "__default") {
      delete refereeOverrides[slot];
    } else {
      refereeOverrides = replaceMatchReferee(baseTeam, refereeOverrides, slot, refereeId);
    }
    const updatedPools = pools.map((pool) => pool.id !== selectedPool?.id ? pool : {
      ...pool,
      matches: pool.matches.map((item) => item.id === match.id ? { ...item, refereeOverrides } : item),
    });
    onUpdateCompetition({ ...competition, pools: updatedPools });
    setEditingRefereeSlot(null);
  }

  function updateTatamiReferee(tatami, slot, refereeId) {
    onUpdateCompetition({
      ...competition,
      refereeAssignments: replaceCompetitionReferee(competition.refereeAssignments || {}, tatami, slot, refereeId),
    });
    setEditingRefereeSlot(null);
  }

  function renderRefereeTeam(tatami, discipline, match = null, editableTatami = false) {
    const baseTeam = competition.refereeAssignments?.[tatami] || {};
    const team = effectiveRefereeAssignments(baseTeam, match?.refereeOverrides || {});
    const refereeSlots = refereeSlotsForDiscipline(discipline);
    const tableCount = TABLE_REFEREE_SLOTS.filter((slot) => refereeName(team[slot]) !== "Non affecté").length;
    const tableStatus = tableCount >= 3 ? "Table complète · 3 arbitres" : tableCount >= 2 ? "Table opérationnelle · 2 arbitres minimum" : `Table incomplète · ${tableCount}/2 minimum`;
    return <div style={{ margin: "12px 0 18px", padding: "14px 16px", border: "2px solid #d7dde5", borderRadius: "12px", background: "#ffffff" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", flexWrap: "wrap", alignItems: "center" }}><strong>ÉQUIPE D’ARBITRAGE · TATAMI {tatami}</strong><span style={{ fontWeight: 700 }}>{match ? "Cliquez sur un nom · remplacement pour ce match" : editableTatami ? `${tableStatus} · Cliquez sur un nom pour modifier l’équipe fixe` : tableStatus}</span></div>
      <div className="match-referee-grid">{refereeSlots.map((slot) => { const assignment = team[slot] || {}; const editingKey = `${match ? match.id : `tatami-${tatami}`}:${slot}`; const isInteractive = Boolean(match || editableTatami); const isEditing = isInteractive && editingRefereeSlot === editingKey; const selectValue = match ? (match.refereeOverrides?.[slot]?.refereeId ? String(match.refereeOverrides[slot].refereeId) : "__default") : String(assignment.refereeId || ""); return <div className={`match-referee-slot ${match?.refereeOverrides?.[slot] ? "replaced" : ""}`} key={slot}><div className="match-referee-role">{slot}</div>{isEditing ? <select autoFocus value={selectValue} onChange={(event) => match ? updateMatchReferee(match, slot, event.target.value) : updateTatamiReferee(tatami, slot, event.target.value)} onBlur={() => setEditingRefereeSlot(null)}>{match && <option value="__default">Affectation fixe · {refereeName(baseTeam[slot])}</option>}{!match && <option value="">— Non affecté —</option>}{referees.map((referee) => <option key={referee.id} value={String(referee.id)}>{referee.nom} {referee.prenom}{referee.club ? ` · ${referee.club}` : ""}</option>)}</select> : isInteractive ? <button type="button" className="match-referee-name" onClick={() => setEditingRefereeSlot(editingKey)}>{refereeName(assignment)}<small>{match?.refereeOverrides?.[slot] ? "Remplaçant pour ce match" : editableTatami ? "Modifier pour les prochains matchs" : "Modifier"}</small></button> : <div className="match-referee-static">{refereeName(assignment)}</div>}</div>; })}</div>
    </div>;
  }
  ...
  async function saveMatch(result) {
    ...
    const refereeTeam = effectiveRefereeAssignments(competition.refereeAssignments?.[selectedMatch.tatami] || {}, selectedMatch.refereeOverrides || {});
  ...
  function groupPassagesByPool(matches) {
    const grouped = new Map();
    matches.forEach(({ pool, match }) => {
      if (!grouped.has(pool.id)) grouped.set(pool.id, { pool, matches: [] });
      grouped.get(pool.id).matches.push({ pool, match });
    });
    return Array.from(grouped.values());
  }

  function poolProgress(matches) {
    const finished = matches.filter(({ match }) => match.statut === "Terminé").length;
    ...
    return {
      ...
      label: finished === total ? "Terminée" : finished > 0 ? "En cours" : "À venir",
    };
  }
  ...
  if (selectedPool && selectedMatch) {
    ...
    return <div className="arbitration-manager"><button className="back-button" onClick={() => setSelected(null)}>← Retour aux matchs</button>{renderRefereeTeam(selectedMatch.tatami, selectedMatch.discipline, selectedMatch)}{selectedMatch.isKataTieBreak && <div style={{ margin: "12px 0 18px", padding: "14px 16px", border: "2px solid #caa52f", borderRadius: "12px", background: "#fff8dc" }}><strong>DÉPARTAGE KATA · TOUR {selectedMatch.kataTieBreakRound}</strong><div style={{ marginTop: "6px" }}>{selectedMatch.kataTieBreakMode}. Le podium restera bloqué jusqu'à la fin du départage.</div></div>}{renderNextPassage(selectedPool, selectedMatch)}{competitionRulesEngine.isKataDiscipline(selectedMatch.discipline) ? <KataSheet key={liveSheetKey} match={matchProps} onSave={saveMatch} /> : <MatchManager key={liveSheetKey} match={matchProps} onSave={saveMatch} />}</div>;
  }
  ...
  return <div className="arbitration-manager"><div className="manager-header"><div><p className="surtitle">ARBITRAGE DIRECT</p><h2>Arbitrage par tatami</h2><p>Sélectionnez un tatami pour suivre la compétition poule par poule. Le choix est mémorisé sur cette tablette.</p></div><div><button className="primary" type="button" onClick={() => setShowControl(true)}>Vue contrôle compétition</button></div></div>{pools.length === 0 ? <div className="empty-state"><h3>Aucun match disponible</h3><p>Générez et validez les poules avant l'arbitrage.</p></div> : <><div className="tatami-tabs" role="tablist" aria-label="Sélection du tatami"><button className={activeTatami === ALL_TATAMIS ? "active" : ""} onClick={() => handleTatamiSelection(ALL_TATAMIS)}>Tous</button>{tatamis.map((tatami) => <button key={tatami} className={activeTatami === tatami ? "active" : ""} onClick={() => handleTatamiSelection(tatami)}>Tatami {tatami}</button>)}</div><div className="tatami-groups">{displayedGroups.map((group) => { const progress = tatamiProgress(group.matches); const current = group.matches.find(({ match }) => match.statut !== "Terminé"); const poolGroups = groupPassagesByPool(group.matches); return <section className="tatami-group" key={group.tatami}><div className="tatami-group-header"><div><p className="surtitle">TATAMI {group.tatami}</p><h3>{progress.finished} terminés · {progress.remaining} restants</h3></div><div className="tatami-progress" aria-label={`${progress.finished} passages terminés sur ${progress.total}`}><span style={{ width: `${progress.total ? (progress.finished / progress.total) * 100 : 0}%` }} /></div></div>{renderRefereeTeam(group.tatami, "", null, true)}<div className="pool-passage-groups">{poolGroups.map(({ pool, matches }) => { const poolState = poolProgress(matches); const category = getCategory(pool.categoryId); const globalCurrentBelongsToPool = matches.some(({ match }) => match.id === current?.match.id); const poolCurrent = poolState.label === "En cours" || globalCurrentBelongsToPool ? matches.find(({ match }) => match.statut !== "Terminé") : null; return <section className={`pool-passage-group ${poolState.label === "Terminée" ? "pool-passage-complete" : ""}`} key={pool.id}><div className="pool-passage-header"><div><p className="surtitle">{disciplineLabel(pool.discipline)} · TATAMI {group.tatami}</p><h3>{pool.nom || category?.nom || "Poule"}</h3><p>{poolState.finished} passage{poolState.finished > 1 ? "s" : ""} terminé{poolState.finished > 1 ? "s" : ""} sur {poolState.total}</p></div><span className={`pool-progress-status ${poolState.label === "Terminée" ? "complete" : poolState.label === "En cours" ? "current" : ""}`}>{poolState.label}</span><div className="pool-progress-track" aria-label={`Progression de la poule : ${poolState.percentage} %`}><span style={{ width: `${poolState.percentage}%` }} /></div></div><div className="competition-list">{matches.map(({ pool: matchPool, match }) => renderMatchCard(matchPool, match, poolCurrent?.match.id))}</div></section>; })}</div></section>; })}</div></>}</div>;
}
