...
function ScorePanel({ score, penalties, onAddPenalty, onRemovePenalty }) { return <div className="score-panel">{["aka", "shiro"].map((side) => <article className={`score-card ${side}`} key={side}><h3>{side.toUpperCase()}</h3><div className="score-values"><p><span>Points positifs</span><strong>{score[`${side}Positive`]}</strong></p><p><span>Points négatifs</span><strong>{score[`${side}Negative`]}</strong></p><p><span>Score total</span><strong>{score[`${side}Total`]}</strong></p></div><div className="penalty-actions"><h4>Pénalités</h4>{PENALTIES.map((penalty) => <button className={`penalty-button ${penalty.id}`} key={penalty.id} type="button" onClick={() => onAddPenalty(side, penalty)}>{penalty.label}</button>)}</div><div className="penalty-list">{penalties[side].map((penalty, index) => <button key={`${penalty.id}-${index}`} type="button" onClick={() => onRemovePenalty(side, index)}>{penalty.label} ×</button>)}</div></article>)}</div>; }
