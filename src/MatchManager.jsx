...
function AssaultCards({ title, rows, onVote, disabled = false, hasMai = false, maiWarnings = { aka: [], shiro: [] }, onAddMai }) { return <div className="assaults-section"><h3>{title}</h3>{rows.map((row, rowIndex) => { const akaMaiCount = (maiWarnings.aka || []).filter((item) => item.assaultIndex === rowIndex).length; const shiroMaiCount = (maiWarnings.shiro || []).filter((item) => item.assaultIndex === rowIndex).length; return <article className="assault-card" key={row.label}><div className="assault-card-header"><span className="assault-label">{row.label}</span><strong className={`result-pill ${voteResult(row.votes).toLowerCase()}`}>{voteResult(row.votes) || "En attente"}</strong></div>{hasMai && <div className="assault-mai-actions" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", margin: "12px 0" }}><button className="penalty-button mai" type="button" disabled={disabled} onClick={() => onAddMai?.("aka", rowIndex)}>AKA MAÏ{akaMaiCount ? ` ×${akaMaiCount}` : ""}</button><button className="penalty-button mai" type="button" disabled={disabled} onClick={() => onAddMai?.("shiro", rowIndex)}>SHIRO MAÏ{shiroMaiCount ? ` ×${shiroMaiCount}` : ""}</button></div>}<div className="judge-vote-grid">{FUKUSHIN.map((judge, judgeIndex) => <div className="judge-vote-card" key={`${row.label}-${judge}`}><strong>{judge}</strong><DecisionButtons value={row.votes[judgeIndex]} disabled={disabled} onChange={(value) => onVote(rowIndex, judgeIndex, value)} /></div>)}</div></article>; })}</div>; }
