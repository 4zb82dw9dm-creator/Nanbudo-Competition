...
import { categoryAgeCompetitionRule, categorySexes } from "./categoryRules";
...
function CategoriesManager({ competition, onUpdateCompetition }) {
  ...
  function updateKataGroup(categoryId, kataGroup) {
    ...
    const ageRule = categoryAgeCompetitionRule(category, competitors);
    if (ageRule && kataGroup !== ageRule.kataGroup) {
      return alert(`${ageRule.ageGroup || "Cette catégorie d’âge"} : ${ageRule.kataGroup} est obligatoire.`);
    }
  ...
  function mergeSelected() {
    ...
    const kataRules = competitionRulesEngine.isKataDiscipline(discipline)
      ? [...new Set(competitorIds.map((id) => categoryAgeCompetitionRule({ competitorIds: [id] }, competitors)?.kataGroup).filter(Boolean))]
      : [];
    if (kataRules.length > 1) return alert("Impossible de fusionner des compétiteurs appartenant à plusieurs groupes d’âge Kata.");
    const kataGroup = competitionRulesEngine.isKataDiscipline(discipline) ? (kataRules[0] || getValidKataGroup(selected.find((category) => category.kataGroup)?.kataGroup)) : "";
