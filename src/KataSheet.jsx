...
function KataSheet({ match, onSave }) {
  ...
  async function save() {
    ...
    await draft.finalize(() => onSave({ kataName, kataScores: notes.map(Number), kataHighestRemoved: result.highest, kataLowestRemoved: result.lowest, kataRetainedScores: result.retained, finalScore: Number(result.average.toFixed(2)), scoreAka: Number(result.average.toFixed(2)), scoreShiro: 0, vainqueur: "aka" }));
  }
  ...
  return <section className="kata-sheet"><div className="manager-header"><div><p className="surtitle">FEUILLE D’ARBITRAGE · KATA</p><h2>Passage Kata</h2><p>{match.categoryName}</p></div><div className="kata-final-card"><span>Moyenne retenue</span><strong>{result ? result.average.toFixed(2) : "--"}</strong></div></div>
    ...
    <fieldset disabled={!draft.editingEnabled} className="draft-fieldset"><label className="kata-select-label">Kata exécuté<select className="tablet-select" value={kataName} onChange={(event) => { draft.markChanged(); setKataName(event.target.value); }}><option value="">{KATA_PLACEHOLDER}</option>{kataOptions.map((kata) => <option key={kata} value={kata}>{kata}</option>)}</select></label>
    <div className="kata-jury"><h3>Jury</h3>{JUDGES.map((judge, index) => <label className="kata-judge-row" key={judge}><span>{judge}</span><select className="tablet-select" value={notes[index]} onChange={(event) => { draft.markChanged(); setNotes(notes.map((note, noteIndex) => noteIndex === index ? event.target.value : note)); }}><option value="">Note...</option>{NOTE_OPTIONS.map((note) => <option key={note} value={note}>{note}</option>)}</select></label>)}</div>
