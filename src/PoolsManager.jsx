...
function PoolsManager({ competition, onUpdateCompetition }) {
  ...
  function completeTieGroup(groupOrder) {
    ...
    const resolvedPool = { ...pool, poolTieBreakOrder: order };
    onUpdateCompetition({ ...competition, pools: pools.map((item) => item.id === pool.id ? { ...resolvedPool, rankingLocked: calculateRanking(resolvedPool), podium: podiumFromPool(resolvedPool), statut: "Terminée" } : item), statut: "Résultats disponibles" });
    setPendingTieBreak(null);
  }
