...
function CompetitionDashboard({ competition, onBack, onUpdateCompetition }) {
  ...
  return <section className="competition-dashboard">
    ...
    {view === "referees" && <div className="competitors-module"><div className="manager-header"><div><p className="surtitle">LISTE AUTOMATIQUE</p><h2>Arbitres</h2><p>{referees.length} arbitre{referees.length > 1 ? "s" : ""} détecté{referees.length > 1 ? "s" : ""} automatiquement depuis les inscriptions.</p></div></div><div className="registrations-table"><table><thead><tr>{["Nom", "Prénom", "Club", "Ville", "Sexe", "Date de naissance", "Grade", "Disciplines", "Fonctions d’arbitrage", "E-mail", "Téléphone"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{referees.map((referee) => <tr key={referee.id}><td>{referee.nom}</td><td>{referee.prenom}</td><td>{referee.club}</td><td>{referee.ville || "—"}</td><td>{referee.sexe}</td><td>{referee.dateNaissance}</td><td>{referee.grade || referee.ceinture}</td><td>{(referee.categoriesInscription || []).join(", ") || "—"}</td><td>{(Array.isArray(referee.fonctionArbitrage) ? referee.fonctionArbitrage : String(referee.roleArbitre || "").split(",")).filter(Boolean).join(", ")}</td><td>{referee.email}</td><td>{referee.telephoneResponsable || "—"}</td></tr>)}</tbody></table></div></div>}
