...
import { exportRegistrationsCsv, exportRegistrationsPdf } from "./registrationExports";
import { DEMO_COMPETITION_MARKER, simulateCompleteTestCompetition } from "./demoCompetitionData";
import { publicRegistrationUrl } from "./routing";
export { REGISTRATION_EXPORT_COLUMNS } from "./registrationExports";
...
export const INITIAL_REGISTRATION_FORM = { club: "", ville: "", responsableClub: "", email: "", telephoneResponsable: "", participants: Array.from({ length: 15 }, () => ({ ...INITIAL_PARTICIPANT_ROW })) };
...
function CompetitionDashboard({ competition, onBack, onUpdateCompetition }) {
  ...
  function simulateDemoResults() {
    ...
    if (window.confirm("Remplir tous les résultats de cette compétition de démonstration ?")) onUpdateCompetition(simulateCompleteTestCompetition(competition));
  }
  ...
  const clubRegistrationUrl = publicRegistrationUrl(window.location.origin, import.meta.env.BASE_URL, competition.slug);
  const registrationsOpen = competition.statut === "Inscriptions ouvertes";
  function toggleRegistrations() { onUpdateCompetition({ ...competition, statut: registrationsOpen ? "Inscriptions fermées" : "Inscriptions ouvertes" }); }
  ...
  return <section className="competition-dashboard">
    ...
    <div className="competition-dashboard-header"><div><p className="surtitle">CYCLE DE COMPÉTITION</p><h2>{competition.nom}</h2><p>{competition.lieu || "Lieu à définir"} · {competition.date || "Date à définir"}</p><p className="public-link">Lien clubs : <a href={clubRegistrationUrl}><code>{clubRegistrationUrl}</code></a></p><p><strong>{registrationsOpen ? "Inscriptions ouvertes" : "Inscriptions fermées"}</strong></p><button className="manage-button" type="button" onClick={toggleRegistrations}>{registrationsOpen ? "Fermer les inscriptions" : "Ouvrir les inscriptions"}</button>{competition.demoMarker === DEMO_COMPETITION_MARKER && <button className="primary" type="button" onClick={simulateDemoResults}>Simuler tous les résultats</button>}</div><span className="status">{competition.statut}</span></div>
    <nav className="competition-menu">{[["dashboard","Tableau de bord"],["competitors","Inscriptions"],["referees","Arbitres"],["categories","Catégories"],["poules","Poules / Tableau"],["planning","Planning"],["arbitrage","Arbitrage"],["results","Résultats"],["statistics","Statistiques"]].map(([id,label]) => <button key={id} className={view === id ? "active" : ""} onClick={() => setView(id)}>{label}</button>)}</nav>
    ...
    {view === "competitors" && <div className="competitors-module"><div className="manager-header"><div><p className="surtitle">TABLEAU DES INSCRITS</p><h2>Inscriptions</h2><p>{competitors.length} inscription{competitors.length > 1 ? "s" : ""} visible{competitors.length > 1 ? "s" : ""} en temps réel.</p></div><button className="primary" onClick={() => setShowForm((current) => !current)}>{showForm ? "Annuler" : "+ Inscrire"}</button></div>{showForm && <RegistrationForm form={form} onClubChange={handleClubChange} onParticipantChange={handleParticipantChange} onClearRows={clearParticipantRows} onAddRows={addParticipantRows} onSubmit={saveCompetitor} submitLabel={editingId ? "Enregistrer" : "Enregistrer l’inscription"} />}<div className="registration-actions"><button className="primary" type="button" onClick={closeRegistrations}>Créer les catégories</button><button type="button" onClick={() => exportRegistrationsCsv(competition)}>Exporter CSV</button><button type="button" onClick={() => exportRegistrationsPdf(competition)}>Exporter PDF</button></div><div className="filters"><input placeholder="Rechercher par nom" value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} /><select value={filters.club} onChange={(event) => setFilters({ ...filters, club: event.target.value })}><option value="">Tous les clubs</option>{clubs.map((club) => <option key={club} value={club}>{club}</option>)}</select><select value={filters.age} onChange={(event) => setFilters({ ...filters, age: event.target.value })}><option value="">Tous les âges</option>{AGE_OPTIONS.map((age) => <option key={age} value={age}>{age}</option>)}</select><select value={filters.ceinture} onChange={(event) => setFilters({ ...filters, ceinture: event.target.value })}><option value="">Toutes ceintures</option>{BELT_OPTIONS.map((belt) => <option key={belt} value={belt}>{belt}</option>)}</select><select value={filters.category} onChange={(event) => setFilters({ ...filters, category: event.target.value })}><option value="">Toutes catégories</option>{REGISTRATION_CATEGORIES.map((category) => <option key={category} value={category}>{category}</option>)}</select></div><div className="registrations-table"><table><thead><tr>{[["nom","Nom"],["prenom","Prénom"],["age","Âge"],["ceinture","Ceinture"],["club","Club"],["ville","Ville"],["email","Email"],["responsableClub","Responsable"],["categorieInscription","Catégories"],["statutInscription","Statut"]].map(([key,label]) => <th key={key}><button onClick={() => sortBy(key)}>{label}</button></th>)}<th>Actions</th></tr></thead><tbody>{filteredCompetitors.map((competitor) => <tr key={competitor.id}><td>{competitor.nom}</td><td>{competitor.prenom}</td><td>{competitor.age}</td><td>{competitor.ceinture}</td><td>{competitor.club}</td><td>{competitor.ville}</td><td>{competitor.email}</td><td>{competitor.responsableClub}</td><td>{(competitor.categoriesInscription || [competitor.categorieInscription]).filter(Boolean).join(", ")}</td><td>{competitor.statutInscription || "À valider"}</td><td><button className="manage-button" onClick={() => editCompetitor(competitor)}>Consulter / modifier</button><button className="manage-button" onClick={() => validateCompetitor(competitor.id)}>Valider</button><button className="delete-button" onClick={() => deleteCompetitor(competitor.id)}>Supprimer</button></td></tr>)}</tbody></table></div></div>}
    {view === "referees" && <RefereeManager competition={competition} referees={referees} onUpdateCompetition={onUpdateCompetition} />}
    {view === "planning" && <PlanningManager competition={competition} onUpdateCompetition={onUpdateCompetition} />}{view === "categories" && <CategoriesManager competition={competition} onUpdateCompetition={onUpdateCompetition} />}{view === "poules" && <PoolsManager competition={competition} onUpdateCompetition={onUpdateCompetition} />}{view === "arbitrage" && <ArbitrationManager competition={competition} onUpdateCompetition={onUpdateCompetition} />}{view === "results" && <ResultsManager competition={competition} />}{view === "statistics" && <StatisticsManager competition={competition} />}
  </section>;
}
...
export function RegistrationForm({ form, onClubChange, onParticipantChange, onClearRows, onAddRows, onSubmit, submitLabel = "Enregistrer l’inscription" }) {
  ...
  return <form className="club-registration-form" onSubmit={onSubmit} noValidate>
