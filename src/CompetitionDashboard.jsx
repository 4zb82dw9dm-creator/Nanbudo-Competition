...
export function RegistrationForm({ form, onChange, onToggleCategory, onSubmit, submitLabel = "S’inscrire" }) { return <form className="competition-form registration-form" onSubmit={onSubmit}><div className="form-title"><p className="surtitle">FORMULAIRE CLUB</p><h3>Inscription compétiteur</h3><p>Les clubs utilisent uniquement ce formulaire public sécurisé.</p></div><div className="form-row"><label>Nom *<input name="nom" value={form.nom} onChange={onChange} required /></label><label>Prénom *<input name="prenom" value={form.prenom} onChange={onChange} required /></label></div><div className="form-row"><label>Âge *<select name="age" value={form.age} onChange={onChange} required><option value="">Sélectionner l’âge</option>{AGE_OPTIONS.map((age) => <option key={age} value={age}>{age} ans</option>)}</select></label><label>Ceinture *<select name="ceinture" value={form.ceinture} onChange={onChange} required><option value="">Sélectionner la ceinture</option>{BELT_OPTIONS.map((belt) => <option key={belt} value={belt}>{belt}</option>)}</select></label></div><div className="form-row"><label>Club *<input name="club" value={form.club} onChange={onChange} required /></label><label>E-mail du responsable du club *<input name="email" type="email" value={form.email} onChange={onChange} required /></label></div><label>Responsable du club<input name="responsableClub" value={form.responsableClub} onChange={onChange} /></label><div className="category-checklist"><strong>Catégories d’inscription *</strong><div className="category-options">{REGISTRATION_CATEGORIES.map((category) => <label key={category} className="checkbox-line"><input type="checkbox" checked={form.categoriesInscription.includes(category)} onChange={() => onToggleCategory(category)} /><span>{category}</span></label>)}</div></div><button className="primary" type="submit">{submitLabel}</button></form>; }
