...
import RefereeAssignments from "./RefereeAssignments";
...
function CompetitionDashboard({ competition, onBack, onUpdateCompetition, readOnly = false }) {
  ...
  return <section className={`competition-dashboard ${readOnly ? "read-only" : ""}`}>
    <button className="back-button" type="button" onClick={onBack}>← Retour aux compétitions</button>
    <div className="competition-dashboard-header"><div><p className="surtitle">CYCLE DE COMPÉTITION</p><h2>{competition.nom}</h2><p>{competition.lieu || "Lieu à définir"} · {competition.date || "Date à définir"}</p>{!readOnly && <p className="public-link">Lien clubs : <code>{publicRegistrationUrl}</code></p>}</div><div><span className="status">{readOnly ? "Mode consultation" : competition.statut}</span>{!readOnly && <button type="button" onClick={() => window.confirm("Clôturer cette compétition et la placer dans l’historique ?") && onUpdateCompetition({ ...competition, statut: "Terminée", finishedAt: new Date().toISOString() })}>Clôturer la compétition</button>}</div></div>
    <nav className="competition-menu">{[["dashboard","Tableau de bord"],["competitors","Inscriptions"],["referees","Arbitres"],["assignments","Affectation des arbitres"],["categories","Catégories"],["poules","Poules / Tableau"],["planning","Planning"],["arbitrage","Arbitrage"],["results","Résultats"],["ranking","Classement clubs"],["statistics","Statistiques"],["documents","Documents"]].map(([id,label]) => <button key={id} className={view === id ? "active" : ""} onClick={() => setView(id)}>{label}</button>)}</nav>
    ...
    {view === "assignments" && <RefereeAssignments competition={competition} onUpdateCompetition={onUpdateCompetition} readOnly={readOnly} />}{view === "statistics" && <StatisticsManager competition={competition} />}{view === "ranking" && <ClubRanking competition={competition} />}{view === "documents" && <DocumentsManager competition={competition} />}
  </section>;
}
